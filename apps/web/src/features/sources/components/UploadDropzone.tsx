...
export function UploadDropzone({ disabled, onUpload }: { disabled: boolean; onUpload: (file: File) => Promise<unknown> }) {
  ...
  return <div>
    <button type="button" disabled={disabled} onClick={() => inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragOver(true) }} onDragLeave={() => setDragOver(false)} onDrop={onDrop} className={`flex w-full flex-col items-center gap-2 rounded-xl border border-dashed px-4 py-8 text-sm transition-[background-color,border-color,transform] duration-150 focus-visible:outline-2 focus-visible:outline-focus-ring ${dragOver ? 'border-border-strong bg-raised text-fg' : 'border-border-strong bg-surface text-fg hover:border-border-strong hover:bg-raised-hover'} ${disabled ? 'cursor-not-allowed opacity-50' : ''}`}>
      <span className="flex size-9 items-center justify-center rounded-lg border border-border bg-raised text-fg"><CloudUpload size={18} strokeWidth={1.75} aria-hidden="true" /></span><span className="font-medium">Drop files here, or click to choose</span><span className="text-xs text-fg-muted">PDF, Office, text — up to 20 MB each</span>
    </button>
