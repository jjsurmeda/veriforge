...
export function SignupPage() {
  ...
  return (
    <AuthShell title="Create your account" description="Start a private evidence workspace in less than a minute." footer={<Link to="/login" className="rounded-md font-medium text-accent hover:text-foreground focus-visible:outline-2 focus-visible:outline-accent">Already have an account</Link>}>
      <form onSubmit={(event) => void onSubmit(event)} className="space-y-4">
        ...
        <label className="relative block"><UserPlus size={16} strokeWidth={1.75} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input name="password" aria-label="Password" type="password" required minLength={8} autoComplete="new-password" placeholder="Password (8+ characters)" className="h-11 w-full rounded-lg border border-border bg-background pl-10 pr-3 text-sm text-foreground placeholder:text-muted-foreground transition-colors duration-150 hover:border-border-strong focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent" /></label>
        {error && <p role="alert" className="rounded-lg border border-danger/30 bg-danger-soft px-3 py-2 text-sm text-danger">{error}</p>}
        <button type="submit" disabled={busy} className="pressable inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-on-primary hover:bg-primary-strong disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><UserPlus size={16} strokeWidth={1.75} aria-hidden="true" />Sign up</button>
      </form>
    </AuthShell>
  )
}
